...
def combine_scenario_name(name1, name2):
    ...
    for info in name2.split("_"):
        actors_info.append(info[2:])
    actors_info = [
        f"{index:02}{info}" for index, info in enumerate(actors_info, start=1)
    ]
...
def combine_yaml(yaml1, yaml2, combined_scenario_name, mode="agent"):
    ...
    combined_yaml = {
        ...
        "Ego": yaml1["Ego"],
        "Map": yaml1["Map"],
        "Actors": {"Agents": combined_agents, "Pedestrians": combined_pedestrians},
    }
...
def combine_csv(csv1, csv2, combined_scenario_name, mode="agent"):
    ...
    for index, col in enumerate(data2[0]):
        if "Agent" in col:
            ...
            data2[0][index] = f"Agent{current_agent_count+agent_count}_" + "_".join(
                col.split("_")[1:]
            )
    ...
    # new scenario id
    combined_csv[1][data1[0].index("scenario_id")] = combined_scenario_name.split("_")[
        -1
    ]
    ...
    # combine description
    combined_csv[1][data1[0].index("description")] = (
        data1[1][data1[0].index("description")]
        + "; "
        + data2[1][data2[0].index("description")]
    )
...
if __name__ == "__main__":
    ...
    scenario_number = (
        len(glob.glob(os.path.join(save_folder, "*.yaml"), recursive=True)) + 1
    )
    combined_scenario_name = f"{combined_scenario_name}_{scenario_number}"

    yaml1 = args.s1 + ".yaml"
    yaml2 = args.s2 + ".yaml"
    combined_yaml, msg = combine_yaml(
        yaml1, yaml2, combined_scenario_name, mode=args.mode
    )
