    ...
    for tag in tags:
        if tag not in all_tags:
            ...
            if (
                ...
                raise Exception(
                    f"Unexpected response while creating tag '{tag}': {response.json()}"
                )
...
def upload_openscenario_file(file_path, filename=None):
    ...
    for scenario in scenarios_doc:
        if (
            scenario.get("openScenario")
            and scenario["openScenario"].get("type") == "File"
            and scenario["openScenario"].get("filename") == filename
        ):
            scenario_uuid = scenario["openScenario"]["openScenario"]
            print(
                f"   [XOSC] Found existing scenario using this file, openScenario UUID: {scenario_uuid}"
            )
            break
    ...
    if scenario_uuid is None:
        ...
        all_openscenarios = dict(
            [(tag["filename"], tag["id"]) for tag in openscenarios_doc]
        )
    ...
    return {"type": "File", "openScenario": scenario_id}
...
def upload(scenario_id):
    ...
    scenario_id = scenario_id.replace(
        ".xosc", ""
    )  # Remove the .xosc extension for consistency
    ...
    current_tags_to_be_used_in_created_scenario = copy.copy(
        tags_to_be_used_in_created_scenario
    )
    ...
    parameters = write_param(
        result
    )  # Should be the same name using in the OpenSCENARIO file
    ...
    valid_conditions = {
        ...
        "conditions": ["FLAG-AV_CONNECTED_Trigger", "EgoHasMoved"],
    }
    ...
    all_scenarios = dict(
        (scenario["scenarioId"], scenario) for scenario in scenarios_doc
    )
    ...
    try:
        ...
        if (
            r.status_code == 201
            and r.json().get("message") == "Scenario successfully created."
        ) or (
...
if __name__ == "__main__":
    ...
    try:
        ...
        if args.sc[0] == "all":
            ...
            print(
                "Scenarios to upload: ",
                len(set(scenario_ids) - set(already_uploaded_scenarios)),
            )
            ...
            success_upload = 0
            with open(
                f"{RUNTIME_DATA_DIR}/success_upload_scenario.txt", "w"
            ) as success_file:
