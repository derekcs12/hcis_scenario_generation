...
egoStraightAsideLeft = {
    "Start_pos": [0, 1, 50, 0, 1],
    "End_pos": [2, -1, 10, 0, 1],
    "Start_speed": 30,
}
egoStraightAsideRight = {
    "Start_pos": [0, 2, 50, 0, 1],
    "End_pos": [2, -2, 10, 0, 1],
    "Start_speed": 30,
}
...
if True:
    ...
    for relative_pos, use_route, shape in [
        ...
        agent1["Start_trigger"] = set_trigger_dict_from_absolute_pos(
            road=egoTriggerAt[0],
            lane=egoTriggerAt[1],
            s=egoTriggerAt[2],
...
if True:
    ...
    for relative_pos, use_route, shape in [
        ("FS-2", None, "Route"),
        ("FR-3", list(config["Center"]), "Curve"),
        ("SR-5", list(config["Center"]), "Curve"),
    ]:
        initRelPostAbbvLon = relative_pos[0]
        initRelPostAbbvLat = relative_pos[1]
        ...
        for end, action in [
            ...
            agent1_lat_event["Dynamic_delay"] = 0
            agent1_lat_event["Dynamic_duration"] = 1
            agent1_lat_event["Dynamic_shape"] = (
                "Curve" if action == "left U turn" else shape
            )
            agent1_lat_event["End"] = end
            agent1_lat_event["Use_route"] = (
                list(config["Center"]) if action == "left U turn" else use_route
            )
...
if True:
    ...
    for relative_pos, start_pos, end_pos in [
        ("FL-1", [1, 1, 10, 0, 1], [3, -1, 15, 0, 1]),
...
if True:
    ...
    for relative_pos in ["FL-M1", "FR-M1", "SL-M1", "SR-M1", "FR-M2", "SR-M2"]:
        ...
        for end, action in [
            ...
            agent1_lat_event["Dynamic_shape"] = (
                "Curve"
                if (action == "left U turn" or "M2" in relative_pos)
                else "Route"
            )
            agent1_lat_event["End"] = end
            agent1_lat_event["Use_route"] = (
