...
def combine_all_scenarios(basic_scenarios_folder):
    ...
    for cata1, cata2 in tqdm(cata_combinations):
        ...
        cata_1s = len(
            glob.glob(
                os.path.join(f"./config/scenario_config/{cata1}", "*.yaml"),
                recursive=False,
            )
        )
        cata_2s = len(
            glob.glob(
                os.path.join(f"./config/scenario_config/{cata2}", "*.yaml"),
                recursive=False,
            )
        )

        # get all combinations of scenarios (scenario's index starts from 1)
        sce_combinations = list(
            itertools.product(range(1, cata_1s + 1), range(1, cata_2s + 1))
        )

        sc_folder_name = combine.combine_scenario_name(cata1, cata2)
        print(
            f"Combining {cata1.ljust(9)} and {cata2.ljust(9)} with {str(len(sce_combinations)).ljust(5)}scenarios. Output folder: {sc_folder_name.ljust(25)}"
        )
        for sce1, sce2 in sce_combinations:
            ...
            scenario_number = (
                len(
                    glob.glob(
                        os.path.join(f"{save_root}/{sc_folder_name}", "*.yaml"),
                        recursive=False,
                    )
                )
                + 1
            ...
            combined_yaml, msg = combine.combine_yaml(
                yaml1, yaml2, combined_scenario_name, mode="agent"
            )
            ...
            combined_csv = combine.combine_csv(
                csv1, csv2, combined_scenario_name, mode="agent"
            )
...
def show_statistics(basic_scenarios_folder):
    ...
    for cata1, cata2 in cata_combinations:
        ...
        cata_1s = len(
            glob.glob(
                os.path.join(f"./scenario_config/{cata1}", "*.yaml"), recursive=False
            )
        )
        cata_2s = len(
            glob.glob(
                os.path.join(f"./scenario_config/{cata2}", "*.yaml"), recursive=False
            )
        )
        ...
        print(
            f"[ {cata1} ] x [ {cata2} ] =  {cata_1s} x {cata_2s}  = {len(sce_combinations)} SCs"
        )
        counter += len(sce_combinations)
