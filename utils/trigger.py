...
def create_EntityTrigger_at_absolutePos(
    Map, Trigger, EntityName, tolerance=2, delay=0, triggerName="EgoApproachInitWp"
):

    road_index, lane_id, s, offset = (
        Trigger["road"],
        Trigger["lane"],
        Trigger["s"],
        Trigger["offset"],
    ...
    return xosc.EntityTrigger(
        ...
        entitycondition=xosc.ReachPositionCondition(
            xosc.LanePosition(s=s, offset=offset, lane_id=lane_id, road_id=road),
            tolerance=tolerance,
        ),
...
def create_flag_trigger(
    variable_name, value="true", delay=0, conditionedge=xosc.ConditionEdge.rising
):
...
def av_connection_timeout_trigger(variable_dict):  # 0
    assert (
        "FLAG-AV_CONNECTION_TIMEOUT" in variable_dict
        and "FLAG-AV_CONNECTED" in variable_dict
    ), "Variable FLAG-AV_CONNECTION_TIMEOUT or FLAG-AV_CONNECTED not defined."
    return create_flag_trigger("FLAG-AV_CONNECTION_TIMEOUT", "true")


def wrong_start_speed_trigger(variable_dict):  # 1
    assert (
        "FLAG-WRONG_START_SPEED" in variable_dict
    ), "Variable FLAG-WRONG_START_SPEED not defined."
    return create_flag_trigger("FLAG-WRONG_START_SPEED", "true")


def ego_reached_end_trigger(variable_dict):  # 2
    assert (
        "FLAG-EGO_REACHED_END" in variable_dict
    ), "Variable FLAG-EGO_REACHED_END not defined."
...
def ego_collision_trigger(variable_dict):  # 4
    assert (
        "FLAG-EGO_COLLISION" in variable_dict
    ), "Variable FLAG-EGO_COLLISION not defined."
...
def create_StoryBoardElement_Trigger(
    name, delay, conditionedge, element, reference, state
):
    return xosc.ValueTrigger(
        ...
        valuecondition=xosc.StoryboardElementStateCondition(
            element=element, reference=reference, state=state
        ),
    )
