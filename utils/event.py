...
def create_Dummy_Event(actorName, actIndex, delay, previousEventName):
    ...
    dummyEvent.add_action(
        f"{actorName}_Event{actIndex}_DummyAction",
...
def create_TransitionDynamics_from_config(
    event, actorName, actIndex, eventType, type="other"
):
    ...
    transition_dynamics = xosc.TransitionDynamics(
        f"${actorName}_{actIndex}_{eventType}_DynamicShape",
        xosc.DynamicsDimension.time,
        f"${actorName}_{actIndex}_{eventType}_DynamicDuration",
    )
...
def generate_Agent_Start_Event(actorName, agent, Map):
    ...
    agentStartTrigger = create_flag_trigger(
        "FLAG-AV_CONNECTED", "true", delay=0, conditionedge=xosc.ConditionEdge.rising
    )
    ...
    advStartSpeedEvent.add_action(
        f"{actorName}_ActivateController",
        xosc.ActivateControllerAction(longitudinal=True, lateral=True),
    )
...
def generate_Speed_Event(
    actorName, actIndex, eventType, event, previousEventName, type="other"
):
    ...
    trigger = create_Trigger_following_previous(
        previousEventName,
        f"${actorName}_{actIndex}_{eventType}_DynamicDelay",
        state="complete",
    )
...
def generate_Cut_Event(
    actorName, actIndex, eventIndex, event, previousEventName, currentPosition
):
    ...
    advgoal = xosc.AbsoluteLaneChangeAction(
        targetLane,
        create_TransitionDynamics_from_config(event, actorName, actIndex, eventIndex),
        target_lane_offset=target_offset,
    )

    trigger = create_Trigger_following_previous(
        previousEventName,
        delay=f"${actorName}_{actIndex}_TA_DynamicDelay",
        state="complete",
    )
...
def generate_Offset_Event(
    actorName, actIndex, eventType, event, previousEventName, currentPosition
):
...
def generate_Position_Event(
    ...
    currentPosition[2] = (
        f"${actorName}_{actIndex-1}_S" if actIndex > 1 else f"${actorName}_S"
    )
    currentPosition[3] = (
        f"${actorName}_{actIndex-1}_TA_Offset"
        if actIndex > 1
        else f"${actorName}_Offset"
    )
    ...
    if event["Dynamic_shape"] == "Straight":
        ...
        nurbs = xosc.Nurbs(2)
        nurbs.add_control_point(
            xosc.ControlPoint(create_LanePosition_from_config(Map, currentPosition))
        )  # 出發點
        ...
        nurbs.add_control_point(
            xosc.ControlPoint(
                create_LanePosition_from_config(Map, currentPosition, s=0)
            )
        )  # 與出發點同道之進入路口點，加這個點軌跡比較自然
        ...
        if event["Use_route"] != None:
            nurbs.add_control_point(
                xosc.ControlPoint(
                    xosc.WorldPosition(road_center[0], road_center[1]), weight=0.5
                )
            )  # 路口中心
        nurbs.add_control_point(
            xosc.ControlPoint(create_LanePosition_from_config(Map, event["End"], s=0))
        )  # 目的地
        nurbs.add_control_point(xosc.ControlPoint(targetPoint))  # 目的地
        if event["Use_route"] != None:
...
def generate_Zigzag_Event(
    ...
    advgoal = xosc.AbsoluteLaneOffsetAction(
        f"${{{amplidute} + {init_offset}}}",
        shape=xosc.DynamicsShapes.sinusoidal,
        maxlatacc=f"${{abs({amplidute}/{period})}}",
    )
    advgoalEvent = xosc.Event(
        f"{actorName}_Event{actIndex}_TrajectoryEvent_1",
    ...
    condition2 = create_StoryBoardElement_Trigger(
        "FollowingPreviosTrigger2",
        0,
        xosc.ConditionEdge.rising,
        "event",
        f"{actorName}_Event{actIndex}_TrajectoryEvent_2",
    ...
    condition3 = create_StoryBoardElement_Trigger(
        "FollowingPreviosTrigger3",
        0,
        xosc.ConditionEdge.rising,
        "event",
        f"{actorName}_Event{actIndex}_TrajectoryEvent_1",
