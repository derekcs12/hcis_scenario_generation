...
class ScenarioTagTree:
    def __init__(self):
        self.tagTree = {
            "ego": {
                ...
                "vehicleLateralActivity": {"mode": None},
            },
            "actors": [],
            "roadLayout": {
                "mode": None,
    ...
    def set_ego_vehicle_activity(self, row):
        self.tagTree["ego"]["vehicleLongitudinalActivity"] = (
            self._set_longitudinal_activity(
                row["ego_long_mode"], row["ego_long_mode_type"]
            )
        )
        self.tagTree["ego"]["vehicleLateralActivity"] = self._set_lateral_activity(
            row["ego_lat_mode"], row["ego_lat_direction"]
        )
    ...
    def add_actor(self, row):
        ...
        for i in range(1, actor_num + 1):
            actor = {
                ...
                "leadVehicle": {"mode": "appearing", "appearingMode": "gapClosing"},
            }
            actor["vehicleLongitudinalActivity"] = self._set_longitudinal_activity(
                row[f"Agent{i}_long_mode"], row[f"Agent{i}_long_mode_type"]
            )
            actor["vehicleLateralActivity"] = self._set_lateral_activity(
                row[f"Agent{i}_lat_mode"], row[f"Agent{i}_lat_direction"]
            )
            self.tagTree["actors"].append(actor)

    def set_road_layout(self, row):
        self.tagTree["roadLayout"]["mode"] = row["road_layout"]
        self.tagTree["roadLayout"][f"{row['road_layout']}Mode"] = row[
            "road_layout_mode"
        ]
...
def write_to_scenario_table(scenario_id, content, file_path="./HCIS_scenarios.csv"):
    ...
    columns = [
        ...
        "Agent1_type",
        "Agent1_long_mode",
        "Agent1_long_mode_type",
        "Agent1_lat_mode",
        "Agent1_lat_direction",
        "Agent1_init_direction",
        "Agent1_init_dynm",
        "Agent1_init_lat_pos",
        "Agent1_init_long_pos",
        "Agent1_S",
        "Agent1_Speed",
        "Agent1_1_SA_EndSpeed",
        "Agent1_1_SA_DynamicDuration",
        "Agent1_1_SA_DynamicDelay",  # BehaviorMode Parameters
        "Agent1_Offset",
    ]
    ...
    if not isZigzag:
        ...
        for name in ["Speed", "1_SA_EndSpeed"]:
            ...
            try:
                ...
                if min_speed < 20.0:
                    print(
                        f"[修正] {scenario_name} {name}的值為 {range}，將其設置為 20.0~20.0"
                    )
...
def clone_behavior_mode_and_wriite_content(
    behavior_type,
    behavior,
    agent1,
    agent1_act,
    ...
    save_config_yaml(
        config, f"./config/scenario_config/{name_attribute}/{next_id}.yaml"
    )

    isZigzag = True if agent1_act["Type"] == "zigzag" else False
    csv_row = generate_csv_content(
        behavior,
        behavior_type,
        descript,
        lateral_behavior,
    ...
    write_to_scenario_table(
        next_id,
        [csv_row],
        file_path=f"./config/scenario_config/{name_attribute}/{next_id}.csv",
    )
...
def set_offset(content, descript, lateral_behavior, agent1_init_direction):
    ...
    def is_motorcycle_left():
        return (
            "R-M2" in descript or "L-M3" in descript
        )  # No M1, since M1 near ego, will cause collision at the beginning

    def is_motorcycle_right():
        return (
            "R-M3" in descript or "L-M2" in descript
        )  # No M1, since M1 near ego, will cause collision at the beginning

    def is_car():
        return not (is_motorcycle_left() or is_motorcycle_right())
    ...
    if "CI" in lateral_behavior or "CO" in lateral_behavior:
        ...
        "U turn" in descript or "turning left" in descript
    ) and agent1_init_direction == "sameAsEgo":
        if "R-M2" in descript:
            agent.update({"Offset": "-1.5~-0.5"})
